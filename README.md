...
Temperatures come from `/sys/class/hwmon`, with thermal-zone readings used to fill gaps. Driver labels identify channels, and critical limits appear only when reported. Faulted sensor readings are omitted with an availability notice. PECI control targets (Tcontrol, Tthrottle, and Tjmax) are excluded from measured temperatures. The CPU headline is the hottest sensor recognized as belonging to the CPU; recognition depends on driver and channel names. PECI DIMM temperatures remain visible as hardware sensors and do not contribute to the CPU headline, even when their driver name includes a CPU socket number. CPU voltage-regulator (VRM) temperatures also remain visible as hardware sensors without contributing to the CPU headline. Readable inputs resolving to the same sysfs file are deduplicated. Other readings are retained when their shared identity cannot be proved, so similar driver names or equal temperatures cannot hide an available package sensor. See the [hwmon interface](https://docs.kernel.org/hwmon/sysfs-interface.html) and [PECI channel definitions](https://docs.kernel.org/hwmon/peci-cputemp.html#sysfs-interface).

Disabled channels are omitted when the driver reports `tempN_enable=0`. IT87 channels marked unused with `tempN_type=0` are also omitted, including aliases of those inputs. Re-enabling a channel restores its readings on the next sample. See the [IT87 sensor configuration](https://docs.kernel.org/hwmon/it87.html#description).
...
Nuvoton's `PCH_CHIP_CPU_MAX_TEMP` remains visible as a hardware sensor but does not contribute to the CPU headline: it reports the maximum of CPU and chipset (MCH) temperatures. Direct CPU channels such as `PCH_CPU_TEMP` still contribute. See section 9.165 of the [NCT6796D datasheet](https://www.nuvoton.com/resource-files/NCT6796D_Datasheet_V0_6.pdf).
